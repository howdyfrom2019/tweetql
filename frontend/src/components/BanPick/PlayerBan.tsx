...
const PlayerBan = ({ patch, blueTeam, image, disabled }: Props) => {
  ...
  return (
    <figure className={`relative w-16 h-16`}>
      ...
      <span className={`absolute w-full h-full bg-200% z-20 top-0 left-0 ${disabled ? '' : 'bg-gradient-to-t animate-ban-select'} ${blueTeam ? 'from-banBlue' : 'from-banRed'}`} />
      <span className={`absolute top-3 left-3 left-0 ${blueTeam ? 'blue-team-diagonal' : 'red-team-diagonal'} z-30`} />
    </figure>
  );
};
